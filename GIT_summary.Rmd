...
Linux kernel, but they can also be smaller scale projects like your own R
development, homework assignments, papers, or thesis. There are many other VCSs
available (subversion and Mercurial are currently used extensively in opens
source projects) but Git is one of the easier ones to set up. It is also well
supported by the GitHub ecosystem.
...
integrated Git support. Checkout other instructions available in
...
```
These two commands only need to be __executed once__. The information you
provide is included in a `.gitconfig` file in your home directory and is used to
label your actions in the Git log.
...
the global options of RStudio and generate an SSH-key which is passed to GitHub.
...
*   Before you execute the commands below (more precisely, 
    `git push -u origin master`), make sure you have made an initial commit
    within the R-project, tracking all initial files (otherwise there is just
    nothing to push to GitHub)
*   Open a shell, then follow the instructions on the new repo page. They'll
    look something like this:
    ```{bash, error = FALSE}
    git remote add origin https://github.com/useRaddictIZ/PGAS_KZ.git
    git push -u origin master
    ```
    The first line tells Git that your local repo has a remote version on GitHub,
    and calls it "origin". The second line pushes all your current work to that
    repo. There is a difference between remote URLs with SSH or HTTPS. HTTPS
    works for me, but alternatively git@github.com/useRaddictIZ/PGAS_KZ.git can
    be used. More help on this can be found on 
    <https://help.github.com/articles/changing-a-remote-s-url/>. Notably, the
...
you'll learn about below in branching.
