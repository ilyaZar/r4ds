...
*   Open a shell, then follow the instructions on the new repo page. They'll
    ...
    GitHub, and calls it "origin". The second line pushes all your current work
    to that repo. There is a difference between remote URLs with SSH or HTTPS.
    HTTPS works for me, but alternatively 
    git@github.com/useRaddictIZ/PGAS_KZ.git can be used. More help on this can
    be found on <https://help.github.com/articles/changing-a-remote-s-url/>. 
    Notably, the types can be switched:
    ```{bash, error = FALSE}
    git remote -v
